...
public class AptitudesService {
 ...
 //borrar y reemplazar aptitudes
    ...
    public List<AptitudesDTO> getAptitudesByIdUsuario(final Long idUsuario) {
        ...
        final List<Aptitudes> idUsuarioAptitudes = aptitudesRepository.findByIdUsuario(candidato);
    ...
    private AptitudesDTO mapToDTO(final Aptitudes aptitudes, final AptitudesDTO aptitudesDTO) {
        aptitudesDTO.setIdAptitud(aptitudes.getIdAptitud());
        aptitudesDTO.setNombreAptitud(aptitudes.getNombreAptitud());
        aptitudesDTO.setIdUsuario(aptitudes.getIdUsuario() == null ? null : aptitudes.getIdUsuario().getIdUsuario());
        return aptitudesDTO;
    }


    private Aptitudes mapToEntity(final AptitudesDTO aptitudesDTO, final Aptitudes aptitudes) {
        aptitudes.setNombreAptitud(aptitudesDTO.getNombreAptitud());
        if (aptitudesDTO.getIdUsuario() != null) {
            candidatoRepository.findById(aptitudesDTO.getIdUsuario())
                    .ifPresent(aptitudes::setIdUsuario);
        }
        return aptitudes;
    }
