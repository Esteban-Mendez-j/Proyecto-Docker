...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const nPostulacion = url.searchParams.get("nPostulacion");
  const params = new URLSearchParams();
  params.append("idUsuario", id);
  if (nPostulacion) params.append("nPostulacion", nPostulacion);
  const apiUrl = `${API_URL}/api/candidatos/perfil?${params.toString()}`;
...
Historial Laboral
</h2> <div class="space-y-6 mt-4"> ${historialLaboral.map((exp) => renderTemplate`<div class="relative pl-6 border-l-2 border-[var(--border)] pb-4"> <div class="absolute -left-[0.4rem] top-0 w-3 h-3 bg-[var(--primary)] rounded-full border-2 border-white"></div> <h3 class="text-base font-semibold text-[var(--text)]"> ${exp.titulo} </h3> <p class="text-sm text-[var(--primary)]">${exp.empresa}</p> </div>`)} </div> </section> </div> </div> </div> ` }), renderComponent($$result, "Footer", $$Footer, { "slot": "footer" }));
}, "C:/Users/Asus/OneDrive/Desktop/Proyecto-Docker/SearchJobs-front/src/pages/perfil/candidato/[id].astro", void 0);

const $$file = "C:/Users/Asus/OneDrive/Desktop/Proyecto-Docker/SearchJobs-front/src/pages/perfil/candidato/[id].astro";
const $$url = "/perfil/candidato/[id]";
